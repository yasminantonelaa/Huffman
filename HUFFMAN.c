...
/**/void generate_map(no_htr *root, char *path, char *map[]); // gera a dicionario de bits da àrvore do huffman
/**/void desaloc_map(char *map[]); // desaloca a memoria das strings do dicionário
/**/u_char set_bit(u_char c, int i); // gera os bits apartir da posição e do conteudo das strings
/**/int convert(char *curr_file, char *converted, char *map[], int tree_size); // cria o arquivo compactado e retorna o tras do cabeçalho
...
/**/void init_stack(u_char *stack, int size); // incializa stacks para funções do cabeçalho
...
{
    char arquivo[257]; // arquivo lido
    char compact_file[257]; // novo arquivo compactado
    ...
    printf("Digite o nome do arquivo compactado SEM A EXTENSÂO e de enter:\n");
    scanf("%251[^\n]", compact_file); // le o arquivo
    getchar(); // consome o \n
    strcat(compact_file, ".huff"); // concatena o nome com a extensão .huff
    ...
    char *map[tam] = {0}; // array de ponteiros para vetor que será usado para armezenar os caminhos que traduzem a árvore

    char empity_path[1] = ""; // inicializa o array do caminho
...
{
    char arquivo[257]; // entrada compactada
    char descompact_file[257]; // saída descompactada

    printf("Digite o nome do arquivo a ser lido e de enter:\n");
    scanf("%256[^\n]", arquivo);
    getchar();

    printf("Digite o nome do arquivo descompactado COM A EXTENSÃO e de enter:\n");
    scanf("%256[^\n]", descompact_file);
    getchar();
...
void generate_map(no_htr *root, char *path, char *map[]) // gera a dicionario de bits da àrvore do huffman
{
    ...
    char left_path[strlen(path) + 2]; // adiciona mais duas posições ao array (uma para o bit e outra para o '\0')
    char right_path[strlen(path) + 2]; // mesmo caso de cima
...
int convert(char *curr_file, char *converted, char *map[], int tree_size) // cria o arquivo compactado e retorna o tras do cabeçalho
{
    ...
    for(int i = 0; i < (tree_size + 2); i++){ // são 2 bytes + tamnho da ávore
    ...
    u_char out_byte = 0; // byte a ser construido 000001
    ...
    while (fread(&curr_byte, sizeof(u_char),1, current_arq) == 1){
        ...
        char *path = map[(int)curr_byte]; // aramzena a string do caminho atual
...
{ 
    ...
    if (byte == '*') { // siguinica que não é uma folha (os bits válidos são folhas)
        ...
        if (byte == '\\' && *locate < tree_size) {
            byte = buffer[(*locate)++]; // consumir o contrabarra
        }
    ...
    while (current_pos < file_size - 1) { // le byte por byte até o penultimo, pois o último contém o trash
        ...
        for (bit_index = 0; bit_index < 8; bit_index++) {
            ...
            if (!current->left && !current->right) { // quado chega a uma folha iimprime o byte e volta para raiz
                ...
                fputc(((dados_no*)current->data)->byte, out); // escreve a folha
                current = root;
            }
        }
    }
