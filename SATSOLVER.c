...
typedef struct { 
    int **clauses;
    int num_vars; // p cnf "3" 2
    int num_clauses; // p cnf 3 "2"
} Formula;
...
typedef struct Node {
    ...
    struct Node *left;  // próxima atribuição com valor 1
    struct Node *right; // próxima atribuição com valor 0
} Node;
...
bool satisfies(Formula *F, Interpretation *I) { 
    ...
    for (int i = 0; i < F->num_clauses; i++) {
        bool clause_sat = false; // clausula entrea por padrão como false
        for (int j = 0; F->clauses[i][j] != 0; j++) { // le a clausula
            int literal = F->clauses[i][j];
            int var = abs(literal); // pega o módulo da clausula porque o módulo da clausula é a posição do indice do array
            int val = I->values[var]; // 0 ou 1 ou -1

            if (val == -1) break; // força a busca por mais caminhos mais ainda não grante que todos sejam percorridos
    ...
    for(int i = 1; i < I->size;i++){ // garante que não ha indefinições
        if(I->values[i] == -1){
            return false;
        }
    }
...
Node* build_tree(Formula *F, Interpretation *I) {
    ...
    for (int i = 1; i <= F->num_vars; i++) { // verifica cada posição do array da interpetraçãos dos literais
        if (I->values[i] == -1) { // o i so pode ser de um  1 pra frente
            var = i;
            break;
        }
    }
    ...
    if (var == -1) return NULL; // retorna nulo se todos os literais ja forma definidos
     // se os literais não forem indefinidos retona o primeiro literal indefinido
    Node *node = malloc(sizeof(Node));
    node->var_index = var;

    I->values[var] = 1; // tenta 1 no literal da posição e cria um filho a esqurda
    node->left = build_tree(F, I);

    I->values[var] = 0; // analogamente tenta 0 e cria um filho a direita
    ...
    return node; // enqaunto não for definido
}
...
// imprime as possibilidades em pre-ordem
    ...
    if (node->left) { // se a arvore vai para a esquerda o literal repesentado pelo nó recebe 1
        assignment[node->var_index] = 1; // marca como 1 a posição
        print_solution_path(node->left, assignment, F, I , kill,root);
    }
    if (node->right) { // analogamnte pela direita recebe 0
...
int main() {
    ...
    I->size = F->num_vars + 1; // porque começa no 1
    I->values = malloc(I->size * sizeof(int));
    for (int i = 1; i < I->size; i++) I->values[i] = -1; // preeche o array da interpretação como -1 (indefinido)
    ...
    kill_execution(root,assignment, F,I); // libreção de memoria do encerramnto final: unsat ou impressão completa
