...
ARQ* upsidedown_img(ARQ *atual) // função de inverter no eixo y
{ 
    ...
    for (int i = atual->img->height - 1; i >= 0; i--) { // mantém as colunas iguais e iverte a ordem das linhas
...
ARQ* invert_img(ARQ *atual){// função de inverter a imagem no eixo x
    ...
    for (int i = 0; i < atual->img->height; i++){ // mantém as linhas iguais e inverte a ordem das colunas
...
int main() {
    ...
    stack_image *deep_copy = alloc_stack_image(); 
    deep_copy->image = arq; 
    push(UNDO, deep_copy); 
